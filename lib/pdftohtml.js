...
function pdftohtml (filename, options) {
  ...
  pdftohtml.prototype.convert = function() {
    var self = this;
    var child = exec('pdf2htmlEX ' + self.options.additional.join(' '), {async:true, silent:true});
    ...
    child.on('exit', function(code, signal){
      if (code === 0) {
        ...
        if (!which('pdf2htmlEX')) {
          echo('Sorry, this script requires pdf2htmlEX.');
          echo('Install it from http://github.com/coolwanglu/pdf2htmlEX');
        }

...
function pdftohtml (filename, outfile, options) {
  ...
  pdftohtml.prototype.convert = function(preset) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      try {
        child = spawn(self.options.bin, self.options.additional);
      }
      catch (err) {
        console.warn("Please install pdf2htmlEX.");
        console.warn("https://github.com/coolwanglu/pdf2htmlEX");
        throw err;
      }
      ...
      child.on('close', (code) => {
        if (code === 0) {
          ...
          reject({code: code, msg:self.options.bin + ' returned an error.', params: self.options.additional});
        }
      });
    });
  };
